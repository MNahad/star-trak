...
use propagator::{Elements, Observer};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  if res.error() {
    ...
    let elements_group = elements_group
      .drain(0..)
      .map(|el| Elements::new(el))
      .collect();
    let sats = propagator::propagate(&elements_group);
    for sat in &sats {
    ...
    let mut observer = Observer::new(0.0, 0.0, 0.0);
    propagator::update_observer_satellites(&mut observer, &sats);
